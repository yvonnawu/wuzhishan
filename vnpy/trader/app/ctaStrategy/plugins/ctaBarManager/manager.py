...
class SymbolBarManager(Logger, BarUtilsMixin):
    ...
    def close_hist_bars(self, freq):
        ...
        for bar in self._gen_bars.get(freq, []):
            self._update_am_bar(am, bar, end_time=bar.datetime + timedelta(seconds=freq2seconds(freq)))
        self.info("品种%s的无可用历史%sK线,更新关闭" % (self._symbol, freq))

    def update_hist_bars(self, freq, bars, end_dt):
        am = self.get_array_manager(freq)
        unit_s = freq2seconds(freq)
        has_uncomplete_bar = False
        if int((end_dt - bars[-1].datetime).total_seconds()) == unit_s:
            new_bars = bars
            self._push_bars[freq] = bars[-1]
        else:
            merged_bar, merged_end_dt = self._merge_unfinished_bar(bars[-1], end_dt, freq)
            complete = int((merged_end_dt - merged_bar.datetime).total_seconds()) == unit_s
            new_bars = (bars[:-1] + [merged_bar]) if complete else bars[:-1]
            if complete:
                self._push_bars[freq] = merged_bar # push this bar
                self.debug("merged bar %s" % merged_bar.__dict__)
            else:
                has_uncomplete_bar = True
        if am.count:
        ...
        if new_bars:
            for bar in new_bars:
                self._update_am_bar(am, bar, end_time=bar.datetime + timedelta(seconds=unit_s))
        ...
        if has_uncomplete_bar:
            self._update_am_bar(am, merged_bar, end_time=merged_end_dt, complete=False)
    ...
    def _merge_unfinished_bar(self, bar, end_dt, freq):
        ...
        return bar, str2dt(str(am.datetimeint[end-1])) + timedelta(minutes=1)
    ...
    def _update_am_bar(self, am, bar, end_time=None, complete=True):
        if am.uncomplete.complete:
            am.updateBar(bar, end_time=end_time, complete=complete)
        else:
            am.updateBar(bar, method="override", end_time=end_time, complete=complete)
    ...
    def _update_uncomplete_bar(self, freq, end_time):
        if self.is_ready(freq):
            current_bar = self._current_bars.get(freq, None)
            if current_bar:
                self._update_am_bar(self._am[freq], current_bar, end_time=end_time, complete=False)
    ...
    def on_tick(self, tick):
        ...
        for freq in list(self._high_freqs.keys()) + list(self._low_freqs.keys()):
            self._update_uncomplete_bar(freq, tick.datetime)
