...
class BarTimer(object):
    def __init__(self, parent, symbol, freq, offset=0):
    ...
    def _is_new_bar_s(self, bar, dt):
        delta = (dt.date() - bar.datetime.date()).days
        delta = delta * 24 + dt.hour - bar.datetime.hour
        delta = delta * 60 + dt.minute - bar.datetime.minute
        delta = delta * 60 + dt.second - bar.datetime.second
    ...
    def is_new_bar(self, bar, dt):
        ...
        return self._f_is_new_bar(bar, dt)  
